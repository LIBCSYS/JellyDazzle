...
# --- 031 spirograph bloom: hypotrochoid with breathing gear ratio, 6-fold mirror
BASE, WINDOW, SUB, TAU = 260, 1000, 36, 380
_BG = _vignette((0.04, 0.01, 0.07))
...
def render(t):
    ...
    w = np.exp(-(tt - n)/TAU)
    th = n*0.11
    q = 2.5 + 1.2*np.sin(n*0.0021)          # winding ratio drifts slowly
    d = 0.35 + 0.25*np.sin(n*0.0009 + 1.7)  # pen offset breathes
    R = 92.0
    x = 0.60*R*np.cos(th) + R*d*np.cos(q*th)
    y = 0.60*R*np.sin(th) - R*d*np.sin(q*th)
    col = _hsv(th*0.0028 + tt*0.0007, 0.95, 1.0)
    acc = np.zeros((H, W, 3))
    for k in range(6):
        a = k*np.pi/3.0
        for sy in (1.0, -1.0):
