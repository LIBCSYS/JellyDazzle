...
# --- 032 harmonograph veil: detuned twin-pendulum web, 4-fold mirror, teal/violet
BASE, WINDOW, SUB, TAU = 280, 1100, 40, 420
_BG = _vignette((0.01, 0.04, 0.06))
...
def render(t):
    ...
    w = np.exp(-(tt - n)/TAU)
    s = n*0.23
    p2 = n*0.0011                      # slow precession of the second pendulum
    x = 74*np.sin(1.0*s + 0.3) + 46*np.sin(2.0025*s + p2)
    y = 62*np.sin(1.503*s) + 40*np.sin(2.996*s + 1.1 + 0.5*p2)
    hue = 0.52 + 0.18*np.sin(n*0.0007) + 0.04*np.sin(s*0.11)
    col = _hsv(hue, 0.80, 1.0)
    acc = np.zeros((H, W, 3))
    for sx in (1.0, -1.0):
        for sy in (1.0, -1.0):
            _splat(acc, sx*x + CX, sy*y + CY, col, w)
