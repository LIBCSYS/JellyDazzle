...
# --- 036 epicycle lace: 3 stacked epicycles (1, -4, 9), phases precess, 5-fold dihedral, cool
BASE, WINDOW, SUB, TAU = 260, 1000, 34, 400
_BG = _vignette((0.0, 0.02, 0.07))
...
def render(t):
    ...
    p1 = n*0.0011
    p2 = n*0.0007 + 2.0
    x = 56*np.cos(th) + 34*np.cos(-4*th + p1) + 18*np.cos(9*th + p2)
    y = 56*np.sin(th) + 34*np.sin(-4*th + p1) + 18*np.sin(9*th + p2)
    hue = 0.45 + 0.28*np.sin(th*0.07 + n*0.0005)
    col = _hsv(hue, 0.85, 1.0)
    acc = np.zeros((H, W, 3))
    for kk in range(5):
        a = kk*2.0*np.pi/5.0
        for sy in (1.0, -1.0):
