                 ...
                 uint32_t seed, const uint32_t *pal)
{
    ...
    for (i = 0; i < P130_NF; i++) {
         ...
         * TEMPORAL REVIEW 2.4.0 (docs/review/04_pattern_temporal.md, F-130):
         * this was a hard threshold (+50 the frame the sample crossed 150),
         * so a family's every line jumped brightness in one frame as the
         * colour walked the palette (delta 2.0 pops on a 0.22 median).
         * Ramp the lift with the deficit instead — same +50 at black,
         * nothing at the threshold, continuous in between. */
        {
            int d = 150 - (cr[i] + cg[i] + cb[i]);
            if (d > 0) { int lift = (d * 50) / 150;
                         cr[i] += lift; cg[i] += lift; cb[i] += lift; }
        }
    }
