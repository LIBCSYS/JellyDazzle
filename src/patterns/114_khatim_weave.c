...
static float p114_inkAv[3], p114_inkBv[3], p114_gndv[3];
        ...
        { float q = h0 + 0.0f;
          ...
          { float f = q - (float)k; int k1 = k + 1 < 256 ? k + 1 : 255;
            for (int c3 = 0; c3 < 3; c3++)
                p114_inkAv[c3] = p114_ramp[k][c3] + (p114_ramp[k1][c3] - p114_ramp[k][c3]) * f; } }
        { float q = h0 + 88.0f;
          q = q - 512.0f * floorf(q * (1.0f / 512.0f));
          if (q > 255.5f) q = 511.0f - q;   /* triangle, may dip <0 */
          if (q < 0.0f) q = 0.0f;
          ...
          { float f = q - (float)k; int k1 = k + 1 < 256 ? k + 1 : 255;
            for (int c3 = 0; c3 < 3; c3++)
                p114_inkBv[c3] = p114_ramp[k][c3] + (p114_ramp[k1][c3] - p114_ramp[k][c3]) * f; } }
        { float q = h0 + 168.0f;
          q = q - 512.0f * floorf(q * (1.0f / 512.0f));
          if (q > 255.5f) q = 511.0f - q;   /* triangle, may dip <0 */
          if (q < 0.0f) q = 0.0f;
          ...
          { float f = q - (float)k; int k1 = k + 1 < 256 ? k + 1 : 255;
            for (int c3 = 0; c3 < 3; c3++)
                p114_gndv[c3] = p114_ramp[k][c3] + (p114_ramp[k1][c3] - p114_ramp[k][c3]) * f; } }
    }
    inkA = p114_inkAv; inkB = p114_inkBv; gnd = p114_gndv;
