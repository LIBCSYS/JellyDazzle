...
void pattern_030(uint32_t *fb, int w, int h, int frame, int sl,
    ...
    /* TEMPORAL REVIEW 2.4.0 (docs/review/04_pattern_temporal.md, F-030):
     * the ring centres were truncated to whole pixels, so a centre gliding
     * at 0.2 px/frame HOPPED one pixel every ~5 frames and the whole ring
     * train lurched with it (delta 5-9 spikes on a 1.0 median, ~100 per
     * 300 frames).  Centres are now Q8 sub-pixel; the octagonal distance is
     * taken in Q8 and the ring phase multiply widened to 64-bit. */
    const int x1 = (int)((cx + 58.0f * sc * cosf(a)) * 256.0f);
    const int y1 = (int)((cy + 40.0f * sc * sinf(0.8f * a)) * 256.0f);
    const int x2 = (int)((cx - 58.0f * sc * cosf(0.9f * a)) * 256.0f);
    const int y2 = (int)((cy - 40.0f * sc * sinf(a + 1.3f)) * 256.0f);
    ...
    for (int y = 0; y < h; y++) {
        int dy1 = (y << 8) - y1, dy2 = (y << 8) - y2;
        uint32_t *row = fb + (long)y * w;
        for (int x = 0; x < w; x++) {
            int xq = x << 8;
            int z1 = (int)((((int64_t)s_octd030(xq - x1, dy1) * KD) >> 16) + q1) & 4095;
            int z2 = (int)((((int64_t)s_octd030(xq - x2, dy2) * KD) >> 16) + q2) & 4095;
