                 ...
                 uint32_t seed, const uint32_t *pal)
{
     ...
     * TEMPORAL REVIEW 2.4.0 (docs/review/04_pattern_temporal.md, F-141):
     * n and m used to be driven independently over overlapping ranges, so
     * every few thousand frames n crossed m — and at n == m the standing wave
     * f = cos(n pi x)cos(m pi y) - cos(m pi x)cos(n pi y) is identically
     * ZERO, i.e. |f| ~ 0 everywhere: the whole plate lit up in a solid wash
     * (measured luma 13 -> 145 -> 6 in 30 frames, per-frame delta 20.4).
     * Drive the second mode as the first PLUS a gap that never closes below
     * 1.6, so the pair keeps morphing through the family without ever
     * degenerating. */
    float n1 = 3.4f + 2.6f * sinf(t * 0.00119f + ph);
    float m1 = n1 + 1.6f + 1.5f * (1.0f + sinf(t * 0.00083f + ph * 1.7f + 2.1f));
    float n2 = 6.7f + 3.3f * sinf(t * 0.00061f + ph * 0.6f + 4.0f);
    float m2 = n2 + 1.6f + 2.0f * (1.0f + sinf(t * 0.00047f + ph * 2.3f + 0.7f));
