                 ...
                 uint32_t seed, const uint32_t *pal)
{
    ...
    ringw = stw;
    /* TEMPORAL REVIEW 2.4.0 (docs/review/04_pattern_temporal.md, F-119):
     * hbase steps one ramp entry every ~27 frames and the whole window's
     * glass snapped hue with it (metronomic delta 1.5 on a 0.36 median).
     * Keep the fraction and lerp the two ramp entries per pixel. */
    float hbf = t * 0.037f + sp * 30.0f;
    float hfr;
    hbase = (int)hbf;
    hfr = hbf - (float)hbase;
    ...
    for (y = 0; y < h; y++) {
        ...
        for (x = 0; x < w; x++) {
            ...
            {
                const float *co2 = p119_ramp[(hi + 1) & 255];
                gl = 0.55f + 0.45f * p119_cos[(int)((r * 0.028f - wave) * 162.97f
                                                    + 4096.5f) & 1023];
                v = stone ? 0.055f : (0.60f + 0.55f * gl);
                ri = (col[0] + (co2[0] - col[0]) * hfr) * v * 255.0f;
                gi = (col[1] + (co2[1] - col[1]) * hfr) * v * 255.0f;
                bi = (col[2] + (co2[2] - col[2]) * hfr) * v * 255.0f;
            }
            {
