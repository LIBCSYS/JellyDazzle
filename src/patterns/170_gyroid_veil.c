                 ...
                 uint32_t seed, const uint32_t *pal)
{
    ...
    /* TEMPORAL REVIEW 2.4.0 (docs/review/04_pattern_temporal.md, F-170):
     * hb = (int)(t*0.05) stepped the whole veil's hue by one ramp entry
     * (80 palette units) every 20 frames in one go.  The ramp base already
     * slides 1.5/frame, so fold hb into it (0.05 entries * 80 = 4/frame)
     * and the hue travels at the same average speed, continuously. */
    p170_build_ramp(pal, (int)(t * 5.5f) + (int)(seed & 32767), 80);
    hb = 0;
    ...
    /* F-170 (cont.): the slice rotation used to be carried in Q6 — a_fix,
     * b_fix truncated to 1/64 of a table cell per pixel — so b_fix could
     * only change by whole units, once every ~10 frames, and when it did
     * the far rows of the surface swung 5-6 table cells in a single frame
     * (delta 2.4 spikes every 9-10 frames on a 0.20 median).  Carry the
     * per-pixel step in Q16 (1024x finer); the table index is px >> 16.
     * ox/oy wrap on the table period (2^16 Q6 units = one full cycle) so
     * the Q16 accumulators cannot overflow whatever `frame` is. */
    a_fix = (int)(scale * cosf(rot) * 10430.38f * 1024.0f);
    b_fix = (int)(scale * sinf(rot) * 10430.38f * 1024.0f);
    ox    = (int)(fmodf(t * 3.1f, 65536.0f) * 1024.0f);
    oy    = (int)(fmodf(t * 1.9f, 65536.0f) * 1024.0f);
    ...
    for (y = 0; y < P170_LH; y++) {
        ...
        for (x = 0; x < P170_LW; x++, px += a_fix, py -= b_fix) {
            int ix = (px >> 16) & 1023, iy = (py >> 16) & 1023;
