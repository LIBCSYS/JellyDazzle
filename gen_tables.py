#!/usr/bin/env python3
# gen_tables.py v3 — builds palette.bin, palette_count.h, sintab.bin, shapes.bin
...
sys.path.insert(0, os.path.join(HERE, 'lab', 'design'))
try:
...
def load_anchor_palettes():
    ...
    ref_path = os.path.join(HERE, 'reference', 'palettes.json')
    try:
    ...
    for f in sorted(glob.glob(os.path.join(HERE, 'lab', 'palettes', 'P*', 'palette.json'))):
...
with open(os.path.join(HERE, 'palette.bin'), 'wb') as f:
...
with open(os.path.join(HERE, 'palette_count.h'), 'w') as f:
    f.write('\n'.join(hdr))
...
draw_path = os.path.join(HERE, 'draw.s')
try:
...
# ---------------- Q14 sine table ----------------
with open(os.path.join(HERE, 'sintab.bin'), 'wb') as f:
    for i in range(256):
...
with open(os.path.join(HERE, 'shapes.bin'), 'wb') as f:
...
pb = os.path.getsize(os.path.join(HERE, 'palette.bin'))
assert pb == JD_SCHEMES * BPS, f'palette size {pb} != {JD_SCHEMES}*{BPS}'
assert os.path.getsize(os.path.join(HERE, 'sintab.bin')) == 1024
assert os.path.getsize(os.path.join(HERE, 'shapes.bin')) == 5 * G * G * 2
...
print('shapes.bin ', os.path.getsize(os.path.join(HERE, 'shapes.bin')),
      'bytes  OK (5 shapes)')
