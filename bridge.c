...
static int probe_step(double budget_ms)
{
    ...
    /* JD_PROBEN=n advances exactly n sub-steps per frame instead of filling
     * the frame's spare time.  Wall-clock budgeting is right for shipping
     * and wrong for QC: it makes the whole schedule machine-speed dependent,
     * so two harnesses on the same start frame diverge and a fault will not
     * reproduce.  Set it and the run is deterministic. */
    static int nfix = -2;
    if (nfix == -2) { const char *e = getenv("JD_PROBEN"); nfix = e ? atoi(e) : -1; }
    int nleft = nfix;
    double t0 = now_ms();
    while (g_probe_i < g_nr) {
        if (pr_rt < 0) {
            /* patterns first: they are the overlay material, and the stack
             * is thin until they are measured */
            int np = g_nr - JD_NASM;
        ...
        if (probe_advance(g_pa, g_pb, jd_palette, g_probe_pix)) {
            pr_rt = -1; g_probe_i++;
        }
        if (nfix >= 0) { if (--nleft <= 0) break; }
        else if (now_ms() - t0 >= budget_ms) break; /* yield mid-routine */
    }
    g_probe_ms += now_ms() - t0;
    if (g_probe_i < g_nr) return 0;
    free(g_pa); free(g_pb); g_pa = g_pb = NULL;
    g_probe_done = 1;
    TR("PROBE complete: %d routines, %.0f ms total\n", g_nr, g_probe_ms);
    return 1;
}
...
static const uint16_t LIFE_S[JD_NSLOT][4] = {
    { 23, 29, 31, 31 },      /* base   — capped so layer-local sl < 2000  */
    { 23, 29, 31, 29 },      /* mid    — long tenancies: fewer draws from  */
    { 17, 19, 23, 19 },      /* accent   the small dark-routine bags, so   */
    { 13, 17, 19, 17 },      /* spark    less repetition and less motion   */
};
...
static void sched_tick(int frame)
{
    ...
    for (int s = 0; s < JD_NBUF; s++) {
        ...
        g_rest[s] = frame + REST_LO[s] +
                    (int)(mix32((uint32_t)frame ^ (uint32_t)s) %
                          (uint32_t)(REST_HI[s] - REST_LO[s] + 1));
    }
    ...
    else if (!handover && g_L[0].live && frame >= g_L[0].t_out - 120) {
    ...
    for (int s = 1; s < JD_NSLOT; s++) {
        ...
        if (handover || (g_hot && s >= 2) || frame - g_last_change < 60) continue;
...
static int mode_override(uint32_t *fb, int w, int h, int frame)
{
    ...
    if (m < JD_NASM) { g_mode = (uint32_t)m; draw_frame(fb, w, h, frame); return 1; }
...
{
    if (!g_ready || w != g_w || h != g_h) {
        if (g_ready) { for (int i = 0; i < JD_NBUF; i++) { free(g_buf[i]); free(g_pal[i]); } g_ready = 0; }
