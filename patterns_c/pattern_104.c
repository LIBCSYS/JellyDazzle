...
static void p104_split(const p104_poly *P, int depth)
{
    p104_poly A, B;
    int i, j, k, m, node = p104_node++ & (P104_NODES - 1);
    float f0, f1, ax, ay, bx, by, area = 0.0f;

    for (i = 0; i < P->n; i++) {
        int nx = (i + 1) % P->n;
        area += P->x[i] * P->y[nx] - P->x[nx] * P->y[i];
    }
    area = area < 0.0f ? -area * 0.5f : area * 0.5f;

    if (depth == 0 || area < 0.055f || P->n < 3 || P->n > P104_MAXV - 2) {
        for (i = 0; i < P->n; i++) {         /* emit: draw the cell outline */
            int nx = (i + 1) % P->n;
            p104_bar(P->x[i], P->y[i], P->x[nx], P->y[nx],
                     p104_hbase + depth * 21 + (int)(P->x[i] * 47.0f));
        }
        return;
    }
     ...
     * usable proportions, which is what the real lattices look like. */
    {
        ...
        for (k = 0; k < P->n; k++) {
            int nx = (k + 1) % P->n;
            float ex = P->x[nx] - P->x[k], ey = P->y[nx] - P->y[k];
            float L = ex * ex + ey * ey;
            if (L > best) { best = L; i = k; }
        }
        for (k = 0; k < P->n; k++) {
            int d = k - i; if (d < 0) d += P->n;
            if (d < 2 || d > P->n - 2) continue;      /* must not be adjacent */
            {
                int nx = (k + 1) % P->n;
                float ex = P->x[nx] - P->x[k], ey = P->y[nx] - P->y[k];
    ...
    /* A = a -> (i+1 .. j) -> b ; B = b -> (j+1 .. i) -> a */
    m = 0;
    A.x[m] = ax; A.y[m] = ay; m++;
    k = (i + 1) % P->n;
    for (;;) {
        A.x[m] = P->x[k]; A.y[m] = P->y[k]; m++;
    ...
    p104_split(&A, depth - 1);
    p104_split(&B, depth - 1);
}
